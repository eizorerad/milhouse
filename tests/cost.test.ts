...
describe("addTokens", () => {
	...
	it("accumulates per-phase tokens when phase provided", () => {
		...
		const scanCost = cost.byPhase.scan;
		expect(scanCost).toBeDefined();
		expect(scanCost?.inputTokens).toBe(1_000_000);
		expect(scanCost?.outputTokens).toBe(100_000);
		expect(scanCost?.cost).toBeCloseTo(3.0 + 1.5, 5);
	});
	...
	it("accumulates multiple calls to the same phase", () => {
		...
		addTokens(cost, 500_000, 50_000, mockConfig, "exec");
		const execCost = cost.byPhase.exec;
		expect(execCost?.inputTokens).toBe(1_000_000);
		expect(execCost?.outputTokens).toBe(100_000);
