...
This Execution Plan was generated by the Consolidator (CDM) agent after merging individual WBS plans.
...
${issueIds
	.map((id) => {
		...
		const title = issue
			? getWorkItemTitle(issue).slice(0, 50) + (getWorkItemTitle(issue).length > 50 ? "..." : "")
			: "Unknown";
		return `| ${id} | ${title} | ${taskCount} |`;
	})
	.join("\n")}`);
	}
