...
/**
 * Generate the markdown content for a single work item
 */
...
| **Rationale** | ${rationale} |`);
	...
	if (issue.scope_impact) {
		parts.push(`| **Scope Impact** | ${issue.scope_impact} |`);
	}
...
export function generateProblemBriefMarkdown(
	...
	const title = options.title || "Work Brief";
...
${overallStatus === "UNVALIDATED" ? "All work items are currently **UNVALIDATED** and require validation before planning." : "Work items have been validated and are ready for planning."}`);
...
`);
	...
	if (overallStatus === "UNVALIDATED") {
		parts.push(`1. Run \`milhouse validate\` to validate each work item
...
`);
		...
		parts.push(`1. Run \`milhouse plan\` to generate WBS for confirmed work items
2. Run \`milhouse consolidate\` to create unified Execution Plan
3. Run \`milhouse exec\` to execute tasks
`);
	} else {
		parts.push(`No actionable work items found.
...
export function regenerateProblemBrief(
	...
	const updatedOptions: ProblemBriefOptions = {
		...
		title: options.title || "Work Brief",
	};
